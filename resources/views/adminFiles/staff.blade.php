...
<h4>Support Staff</h4>
<div class="mb-2">
@if(in_array(5,explode(',',session()->get('LoggedInUser.priviledges'))))
        ...
        <form action="#" id="teachereditform" class="p-2" method="POST" enctype="multipart/form-data">
         <div class="row">
            ...
            <div class="form-group mb-2 d-grid">
             <input type="submit" id="editteachersubmitbtn" class="btn btn-warning btn-sm rounded-0" value="EDIT SUPPORT STAFF">
            </div>
         </div>
        </form>
        </div>
...
<div class="modal fade" id="teacheraddModal" tabindex="-1" aria-labelledby="teacheraddModalLabel">
    ...
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title text-success text-center" id="teacheraddModalLabel">Register Support Staff Member</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
        <form action="#" id="teacherregform" class="p-2" method="POST" enctype="multipart/form-data">
            <div class="row">
                <div class="col-lg-6">
                <input type="number" name="sid" value="{{ session()->get('schooldetails.id') }}" hidden>
                <input hidden type="text" name="role" value="Support Staff" id="role">
                ...
                <div class="form-group mb-3 d-grid">
                    <button type="submit" id="teacheregbtn" class="btn btn-info">REGISTER SUPPORT STAFF</button>
                </div>
                </form>
...
<script>
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.teachers.length == 0) {
                        $('tbody').html('<h5 class="text-center text-danger">No any support staff member registered as at now</h5>');
                    } else {
                        $('tbody').html('');
    ...
    //Teacher Viewing
     $(document).on('click', '#viewbtn',function(e){
        ...
        if (ids.length < 1) {
            alert('Please select a Support Staff to view details');
        } else if(ids.length > 1){
            alert('You can only view one Support Staff at a Time.Select only one Support Staff');
        } else {
            fetchteacher(ids)
    ...
    $(document).on('click', '#teachereditbtn',function(e){
        ...
        if (ids.length < 1) {
            alert('Please select a Support Staff to edit');
        } else if(ids.length > 1){
            alert('You can only edit one Support Staff at a time. Select only one Support Staff');
        } else {
            fetchteacher2(ids)
     ...
     $(document).on('click', '#priviledgesbtn',function(e){
        ...
        if (ids.length < 1) {
            alert('Please select a Support Staff to Assign Priviledges');
        } else if(ids.length > 1){
            alert('You can only assign privildeges to one Support Staff at a time. Select only one Staff Support');
        } else {
            fetchteacher3(ids)
        ...
        if (ids.length < 1) {
            alert('You must select a Support staff(s) to delete')
        } else {
            var confirm = window.confirm(`Are you sure you want to delete this support Staff? You will not be able to revert this action one executed`);
        if (confirm) {
            $.ajax({
                method: 'GET',
    ...
    $(document).on('click', '#deactivatebtn',function(e){
        ...
        if (ids.length < 1) {
            alert('You must select a support staff(s) whose account(s) is/are to be deactivated')
        } else {
            var confirm = window.confirm(`Are you sure you want to Deactivate this support staff Account? Once you deactivate they will not be able to login`);
        if (confirm) {
            $.ajax({
                method: 'GET',
...
$(document).on('click', '#activatebtn',function(e){
        ...
        if (ids.length < 1) {
            alert('You must select a support staff(s) whose account(s) is/are to be activated')
        } else {
            var confirm = window.confirm(`Are you sure you want to activate this support staff Account?`);
        if (confirm) {
            $.ajax({
                method: 'GET',
