        ...
        <tbody id="entermarkstbody">
           @foreach($students as $student)
            <tr id="tablerow" sval="{{ $student->id }}">
                ...
                <td><input send="send" sval="{{ $student->id }}"  readonly type="text" value="{{ $student->AdmissionNo }}" name="admissionnumber[]" id="admissionnumber" class="form-control admissionnumber"></td>
                <td><input send="send" sval="{{ $student->id }}" readonly type="text" value="{{ $student->Fname }} {{ $student->Lname }}" name="name[]" id="name" class="form-control name"></td>
                <td><input send="send" sval="{{ $student->id }}" type="text" value="" name="maxscored[]" id="maxscored" class="form-control maxscored"></td>
                <td>
                    <div class="form-group">
                        <input send="send" sval="{{ $student->id }}" type="text" class="form-control grade" name="grade[]" id="grade">
                        <div class="invalid-feedback"></div>
                    </div>
                </td>
                <td>
                    <div class="form-group">
                        <input send="send" sval="{{ $student->id }}" type="text" class="form-control remarks" name="remarks[]" id="remarks">
                        <div class="invalid-feedback"></div>
                    </div>
                </td>
            </tr>
...
$(document).ready(function(){
    ...
    $(document).on('click','#delbtn',function(e){
        ...
        if (confirm) {
            $.ajax({
                method: 'GET',
                url: `/deletecbcmarks/${selectedbox}/${exam}/${classid}/${sub}`,
                contentType: false,
                processData: false,
                //dataType: 'json',
     ...
     $(document).on('click','#marksaddbtn',function(){
        ...
        $('#withoutmarksadd').addClass('d-none');
         if ($('#subjectdiv').val() == '') {
            ...
            $.ajax({
                method: 'GET',
                url: `/fetchcbcmarks/${exam}/${classid}/${sub}`,
                contentType: false,
                processData: false,
                success: function(res){
                    //$('#subject1').text('');
                    ...
                    var appenddata = '';
                    var appenddata2 = '';
                    appenddata +='<tr deladm="'+(item.AdmissionNo == null ? item.UPI : item.AdmissionNo)+'">';
                    appenddata2 +='<tr>';

                    var stunum = item.AdmissionNo == null ? item.UPI.toString() : item.AdmissionNo.toString();
                    // if (res.student.split(',').includes(item.AdmissionNo)) {
                    if (res.student2.includes(stunum)) {
