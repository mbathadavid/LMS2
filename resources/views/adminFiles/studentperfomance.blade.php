  ...
  <div class="col-lg-6 col-md-8 col-sm-12">
    ...
    @else
      @for ($i = 0; $i < count(explode(',',$student['suborlearningpaths'])); $i++)
        <p class="text-center">{{ explode(',',$student['suborlearningpaths'])[$i] }} <b>(<a class="text-danger text-decoration-none text-center" href="/student-subject-perfomance/{{ $student['id'] }}/{{ explode(',',$student['subids'])[$i] }}">View Perfomance and Reports</a>)</b></p>
                ...
                success: function(res) {
                  ...
                  if (res.schoolsystem == "CBC") {
                    if (res.cbcassessments.length == 0) {
                      ...
                      $.each(res.cbcassessments, function(key,item){
                            ...
                            html += '<td><a class="btn btn-sm btn-success" href="/student-assessment-perfomance/{{ $student["id"] }}/'+item.id+'">Analyze</a></td>';
                            html += '</tr>';
                  ...
                  if (res.scores.length == 0) {
                    ...
                    for (let i = 0; i < res.threads.length; i++) {
                      ...
                      html += '<td><a class="btn btn-sm btn-danger" href="/student-examination-perfomance/{{ $student["id"] }}/'+res.threadids[i]+'">Analyze</a></td>';
                      html += '</tr>';
